...
public class Analyzer {
	...
	public static Result analyzeForChart(String pattern, List<Invoice> incomeInvoices, List<Invoice> outcomeInvoices) {
		...
		if (incomeInvoices.size() + outcomeInvoices.size() <= 0) {
			...
			List<IncomeAndOutcomeAnalysisModel> incomeAndOutcomeAnalysisResult = analysisHelper
					.getIncomeAndOutcomeAnalysisResult();
		...
		result.add("dates", dates).add("incomes", incomes).add("outcomes", outcomes);
		return result;
	}

	public static Result analyzeForReport(String pattern, List<Invoice> incomeInvoices, List<Invoice> outcomeInvoices,
		...
		if (incomeInvoices.size() + outcomeInvoices.size() <= 0) {
			...
			for (IncomeAndOutcomeAnalysisModel m : incomeAndOutcomeAnalysisResult) {
				...
				incomes.add(new BigDecimal(m.getIncomes()).setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue());
				outcomes.add(new BigDecimal(m.getOutcomes()).setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue());
				balances.add(new BigDecimal(m.getOutcomes() - m.getIncomes()).setScale(2, BigDecimal.ROUND_HALF_UP)
						.doubleValue());
			}

			// 进销项产品数据分析
			List<List<ProductAnalysisModel>> incomeAndOutcomeProductAnalysis = analysisHelper
			...
			balances.add(new BigDecimal(outcomeProductTotals.get(outcomeProductTotals.size() - 1)
					- incomeProductTotals.get(incomeProductTotals.size() - 1)).setScale(2, BigDecimal.ROUND_HALF_UP)
							.doubleValue());
			...
			if (pattern.contains("dd")) { // 日度
				dateString = dayFormat.format(startDate) + "至" + dayFormat.format(endDate);
				calendar.set(Calendar.DATE, calendar.get(Calendar.DATE) + 1);
				preDate = dateFormat.format(calendar.getTime());
			} else if (pattern.contains("MM")) { // 月度
				dateString = monthFormat.format(startDate) + "至" + monthFormat.format(endDate);
				calendar.set(Calendar.MONTH, calendar.get(Calendar.MONTH) + 1);
				preDate = dateFormat.format(calendar.getTime());
			} else { // 年度
				dateString = yearFormat.format(startDate) + "至" + yearFormat.format(endDate);
			...
			outcomeComment = compareIncomeOrOutcome(outcomeAmounts, outcomeProductTotals, dateString, "销项",
					outcomeNames);

			compareComment.append("进销项对比分析：由上述的数据可知，在").append(dateString).append("，企业的总进项额为")
					.append(incomeProductTotals.get(incomeProductTotals.size() - 1)).append("元，").append("总销项额为")
					.append(outcomeProductTotals.get(outcomeProductTotals.size() - 1)).append("元，").append("进、销项差值为：")
					.append(Math.abs(balances.get(balances.size() - 1))).append("元。");
		...
		Result result = new Result(successful);
		result.setMessage(message);
		result.add("balances", balances).add("income_product_totals", incomeProductTotals)
				.add("outcome_product_totals", outcomeProductTotals).add("income_names", incomeNames)
				.add("outcome_names", outcomeNames).add("income_amounts", incomeAmounts)
				.add("outcome_amounts", outcomeAmounts).add("dates", dates).add("incomes", incomes)
				.add("outcomes", outcomes).add("income_comments", incomeComment).add("outcome_comments", outcomeComment)
				.add("compare_comments", compareComment.toString()).add("pre_date", preDate)
				.add("pre_income", preIncome).add("pre_outcome", preOutcome).add("pre_balance",
						new BigDecimal(preOutcome - preIncome).setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue());
		return result;
	}
	...
	private static String compareIncomeOrOutcome(List<List<Double>> amounts, List<Double> productTotals,
		...
		comments.append("企业在").append(dateString).append("时间段内，总计").append(type)
				.append(productTotals.get(productTotals.size() - 1)).append("元，总体在").append(dateString)
				.append("时间段内呈现");
